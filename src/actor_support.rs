...
impl TryFrom<ServiceMsg> for VsmActorMsg {
    ...
    fn try_from(value: ServiceMsg) -> Result<Self, Self::Error> {
        match value {
            ServiceMsg::Channel(inner) => Ok(inner),
            other => Err(other),
        }
    }
}
...
impl ServiceState {
    pub fn new(kind: ServiceKind, config: Value) -> Self {
        ...
        Self {
            kind,
            data: json!({"id": id, "config": config, "started_at": now_json(), "status":"running"}),
            history: Vec::new(),
        }
    }
...
impl Actor for ServiceActor {
    ...
    async fn handle(
        ...
        match msg {
            ...
            ServiceMsg::Cast(op, payload) => {
                let _ = handle_service_cast(self.kind, &op, payload, state).await;
            }
            ServiceMsg::Channel(msg) => {
                let payload = match msg {
                    VsmActorMsg::ChannelMessage(message)
                    | VsmActorMsg::AlgedonicSignal(message) => {
                        serde_json::to_value(message).unwrap_or(Value::Null)
                    }
                };
                state.record(
                    json!({"event":"channel_message", "payload": payload, "timestamp": now_json()}),
                );
            }
            ServiceMsg::Tick(kind) => {
                state.record(json!({"event":"tick", "kind": kind, "timestamp": now_json()}))
            }
        }
        Ok(())
    }
...
async fn subscribe(
    myself: ActorRef<ServiceMsg>,
    channel: ChannelKind,
    subscriber_id: &'static str,
) -> Result<(), ActorProcessingErr> {
    crate::channels::subscribe(channel, subscriber_id, myself.get_derived::<VsmActorMsg>())
        .await
        .map_err(|e| -> ActorProcessingErr { e.into() })
}

pub async fn call_service(name: &str, op: &str, payload: Value) -> VsmResult<Value> {
    let actor = ActorRef::<ServiceMsg>::where_is(name.to_string())
        .ok_or_else(|| VsmError::ActorUnavailable(name.to_string()))?;
    call_t!(actor, ServiceMsg::Call, 5_000, op.to_string(), payload)
        .map_err(|err| VsmError::Runtime(err.to_string()))?
}
...
pub fn cast_service(name: &str, op: &str, payload: Value) -> VsmResult<()> {
    let actor = ActorRef::<ServiceMsg>::where_is(name.to_string())
        .ok_or_else(|| VsmError::ActorUnavailable(name.to_string()))?;
    actor
        .send_message(ServiceMsg::Cast(op.to_string(), payload))
        .map_err(|err| VsmError::Runtime(err.to_string()))
}
...
pub fn service_child(name: &'static str, kind: ServiceKind, args: Value) -> ChildSpec {
    ChildSpec {
        ...
        spawn_fn: SpawnFn::new(move |supervisor_cell, child_id| {
            let args = args.clone();
            async move { spawn_service(supervisor_cell, child_id, kind, args).await }
        }),
        backoff_fn: None,
...
async fn spawn_service(
    ...
    let (actor, _join) =
        Supervisor::spawn_linked(child_id, ServiceActor { kind }, args, supervisor_cell).await?;
    Ok(actor.get_cell())
}

pub async fn handle_service_call(
    kind: ServiceKind,
    op: &str,
    payload: Value,
    ...
    state.record(
        json!({"event":"call", "op": op, "payload": payload.clone(), "timestamp": now_json()}),
    );
    match kind {
...
pub async fn handle_service_cast(
    kind: ServiceKind,
    op: &str,
    payload: Value,
    state: &mut ServiceState,
    ...
    state.record(
        json!({"event":"cast", "op": op, "payload": payload.clone(), "timestamp": now_json()}),
    );
    match kind {
