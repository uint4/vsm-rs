...
pub fn perform_audit(unit_ids: &[String], audit_type: &str, system_state: &Value) -> Value {
    let results: Vec<Value> = unit_ids.iter().map(|unit| audit_single_unit(unit, audit_type, system_state)).collect();
...
pub fn analyze_audit_patterns(audit_results: &[Value]) -> Value {
    let compliance: Vec<f64> = audit_results.iter().filter_map(|r| r.get("compliance_score").and_then(Value::as_f64)).collect();
    let issue_count: usize = audit_results.iter().map(|r| r.get("issues").and_then(Value::as_array).map(Vec::len).unwrap_or(0)).sum();
...
pub fn generate_audit_report(audit_results: &[Value]) -> Value {
    ...
    let avg = patterns.get("avg_compliance").and_then(Value::as_f64).unwrap_or(1.0);
...
fn audit_single_unit(unit_id:&str, audit_type:&str, state:&Value)->Value{
    let unit = state.get("units").and_then(|u| u.get(unit_id)).cloned().unwrap_or_else(|| json!({}));
    let mut findings=Vec::new();
    let load=unit.get("load").and_then(Value::as_f64).unwrap_or(0.0);
    if load > 0.9 { findings.push(json!({"category":"performance", "severity":"high", "message":"high load"})); }
    let errors=unit.get("errors").and_then(Value::as_u64).unwrap_or(0);
    if errors > 0 { findings.push(json!({"category":"error_handling", "severity":"medium", "message":"recent errors", "count": errors})); }
