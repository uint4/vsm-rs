//! Audit helper algorithms for System 3* defaults.
//!
//! These JSON helpers are examples retained from the earlier port. Typed audit
//! behavior is supplied through the public `Auditor` role.
...
pub fn schedule_audits(units: &[Value], risk_scores: &Value) -> Value {
    let schedules: Vec<Value> = units
        .iter()
        .map(|unit| {
            let id = unit.get("id").and_then(Value::as_str).unwrap_or("unknown");
            let risk = risk_scores.get(id).and_then(Value::as_f64).unwrap_or(0.5);
            json!({
                "unit_id": id,
                "audit_type": if risk > 0.75 {
                    "comprehensive"
                } else if risk > 0.4 {
                    "focused"
                } else {
...
pub fn analyze_audit_patterns(audit_results: &[Value]) -> Value {
    let compliance: Vec<f64> = audit_results
        .iter()
        .filter_map(|result| result.get("compliance_score").and_then(Value::as_f64))
        .collect();
    let issue_count: usize = audit_results
        ...
        .sum();
    json!({
        "audit_count": audit_results.len(),
        "avg_compliance": mean(&compliance),
...
pub fn generate_audit_report(audit_results: &[Value]) -> Value {
    ...
    let average = patterns
        .get("avg_compliance")
        .and_then(Value::as_f64)
        .unwrap_or(1.0);
    json!({
        "generated_at": now_json(),
        "summary": {
            "units_audited": audit_results.len(),
            "average_compliance": average,
            "overall_status": if average >= 0.8 {
                "healthy"
            } else if average >= 0.6 {
                "watch"
            } else {
