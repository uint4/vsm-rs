...
pub fn scan_environment(sources: &[Value], options: &Value) -> Value {
    let signals: Vec<Value> = sources
        .iter()
        .flat_map(|s| extract_signals(s, options))
        .collect();
...
pub fn detect_changes(current: &Value, previous: &Value) -> Value {
    ...
    if let (Some(c), Some(p)) = (current.as_object(), previous.as_object()) {
        for (k, cv) in c {
            ...
            if cv != pv {
                changes.push(
                    json!({"field": k, "from": pv, "to": cv, "magnitude": magnitude(cv, pv)}),
                );
            }
        }
    }
...
pub fn classify_signals(signals: &[Value]) -> Value {
    ...
    for s in signals {
        let val = s
            .get("value")
            .and_then(as_f64)
            .or_else(|| s.get("score").and_then(as_f64))
            .unwrap_or(0.0);
        if val > 0.65 {
            opportunities.push(s.clone());
        } else if val < -0.35 {
            threats.push(s.clone());
        } else {
            weak.push(s.clone());
        }
    }
...
pub fn monitor_trends(history: &[Value]) -> Value {
    let values: Vec<f64> = history
        .iter()
        .filter_map(|v| v.get("value").and_then(as_f64))
        .collect();
...
pub async fn actor_call(
    ...
    match op {
        "scan" | "scan_environment" => Ok(scan_environment(
            &payload
                .get("sources")
        ...
        "detect_changes" => Ok(detect_changes(
            payload.get("current").unwrap_or(&Value::Null),
            payload.get("previous").unwrap_or(&Value::Null),
        )),
        "classify" => Ok(classify_signals(
            &payload.as_array().cloned().unwrap_or_default(),
        )),
        "trends" => Ok(monitor_trends(
            &payload.as_array().cloned().unwrap_or_default(),
        )),
...
fn extract_signals(source: &Value, _options: &Value) -> Vec<Value> {
    source.get("signals").and_then(Value::as_array).cloned().unwrap_or_else(|| vec![json!({"source": source.get("id").cloned().unwrap_or(Value::Null), "value": source.get("value").cloned().unwrap_or(json!(0.0)), "timestamp": now_json()})])
}
