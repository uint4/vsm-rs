...
pub fn correlate(data: &[Value], _options: &Value) -> Value {
    let xs: Vec<f64> = data
        .iter()
        .filter_map(|v| v.get("x").and_then(as_f64))
        .collect();
    let ys: Vec<f64> = data
        .iter()
        .filter_map(|v| v.get("y").and_then(as_f64))
        .collect();
    ...
    let corr = if n == 0 {
        ...
        let mx = mean(&xs[..n]);
        let my = mean(&ys[..n]);
        let num = (0..n).map(|i| (xs[i] - mx) * (ys[i] - my)).sum::<f64>();
        let den = ((0..n).map(|i| (xs[i] - mx).powi(2)).sum::<f64>()
            * (0..n).map(|i| (ys[i] - my).powi(2)).sum::<f64>())
        .sqrt()
        .max(1e-9);
        num / den
    };
...
fn trend_analysis(data: &[Value]) -> Value {
    let vals: Vec<f64> = data
        ...
        .filter_map(|v| v.get("value").and_then(as_f64).or_else(|| as_f64(v)))
        .collect();
    let trend = vals.last().unwrap_or(&0.0) - vals.first().unwrap_or(&0.0);
    json!({"trend":trend,"direction":if trend>0.0{"up"}else if trend<0.0{"down"}else{"flat"},"points":vals.len()})
}
fn summary(data: &[Value]) -> Value {
    let vals: Vec<f64> = data
        ...
        .filter_map(|v| v.get("value").and_then(as_f64).or_else(|| as_f64(v)))
        .collect();
    json!({"count":data.len(),"mean":mean(&vals),"std_dev":std_dev(&vals)})
}
