...
pub fn calculate_correlation(signal1: &AlgedonicSignal, signal2: &AlgedonicSignal) -> f64 {
    let same_source = if signal1.source == signal2.source { 0.35 } else { 0.0 };
    ...
    let same_kind = if signal1.kind == signal2.kind { 0.15 } else { 0.0 };
    same_source + temporal + priority_similarity + same_kind
}

pub fn cluster_signals(signals: &[AlgedonicSignal], options: &Value) -> Value {
    let by = options.get("by").and_then(|v| v.as_str()).unwrap_or("source");
    let mut groups = serde_json::Map::new();
    for signal in signals {
        let key = match by { "kind" => format!("{:?}", signal.kind), "severity" => format!("{:?}", signal.severity), _ => signal.source.clone() };
        groups.entry(key).or_insert(json!([])).as_array_mut().unwrap().push(json!(signal));
    }
    Value::Object(groups)
}

pub fn detect_anomalies(signals: &[AlgedonicSignal], _historical_patterns: &Value) -> Vec<AlgedonicSignal> {
    let priorities: Vec<f64> = signals.iter().map(|s| s.priority).collect();
    let avg = mean(&priorities);
    let sd = std_dev(&priorities).max(0.05);
    signals.iter().filter(|s| s.priority > avg + (2.0 * sd)).cloned().collect()
}
