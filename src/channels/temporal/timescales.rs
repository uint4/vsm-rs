...
pub fn initialize(config: &Value) -> Timescales {
    ...
    Timescales {
        windows: scales.into_iter().map(|s| (s, VecDeque::new())).collect(),
        max_points,
    }
}
...
pub fn update(mut timescales: Timescales, metric: TemporalMetric) -> Timescales {
    for buffer in timescales.windows.values_mut() {
        ...
        while buffer.len() > timescales.max_points {
            buffer.pop_back();
        }
    }
    timescales
...
pub fn get_variety(timescales: &Timescales, scale: &str) -> Value {
    let points: Vec<f64> = timescales
        .windows
        .get(scale)
        .map(|b| b.iter().flat_map(|m| numeric_values(&m.data)).collect())
        .unwrap_or_default();
...
pub fn get_statistics(timescales: &Timescales) -> Value {
    ...
    for scale in timescales.windows.keys() {
        obj.insert(scale.clone(), get_variety(timescales, scale));
    }
    Value::Object(obj)
}
