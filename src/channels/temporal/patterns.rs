...
pub fn detect_anomalies(data: &[f64]) -> Value {
    ...
    let anomalies: Vec<_> = data.iter().enumerate().filter(|(_, v)| (**v - avg).abs() > 3.0 * sd).map(|(i, v)| json!({"index": i, "value": v})).collect();
    json!(anomalies)
}
