...
pub fn evaluate_alignment(state: &ServiceState, proposal: &Value) -> Value {
    let id = state
        ...
        .get("identity")
        .cloned()
        .unwrap_or_else(identity::default_identity);
    let vals = state
        .data
        .get("values")
        .cloned()
        .unwrap_or_else(values::default_values);
...
pub fn handle_crisis(state: &mut ServiceState, crisis_info: Value) -> Value {
    let severity = crisis_info
        .get("severity")
        .and_then(Value::as_str)
        .unwrap_or("medium")
        .to_string();
    let decision = decisions::make_decision(
        state,
        json!({"subject":"crisis_response", "options":[{"name":"stabilize", "score":0.9},{"name":"monitor", "score":0.5}], "criteria":[{"name":"score", "weight":1.0}], "crisis": crisis_info}),
    );
...
pub async fn actor_call(op: &str, payload: Value, state: &mut ServiceState) -> VsmResult<Value> {
    match op {
        ...
        "set_policy" => {
            let policy_area = payload
                .get("policy_area")
                .and_then(Value::as_str)
                .unwrap_or("general")
                .to_string();
            let policy_details = payload
                .get("policy_details")
                .cloned()
...
pub async fn make_decision(request: Value) -> VsmResult<Value> {
    let decision =
        crate::actor_support::call_service(crate::names::SYSTEM5_POLICY, "make_decision", request)
            .await?;
...
pub async fn set_policy_area(
    policy_area: impl Into<String>,
