...
pub fn detect_imbalance(allocations: &Value) -> Value {
    ...
    for (unit, alloc) in &allocations {
        ...
        if score > avg * 1.2 {
            over.push(json!({"unit_id": unit, "score": score}));
        }
        if score < avg * 0.8 {
            under.push(json!({"unit_id": unit, "score": score}));
        }
    }
...
fn total_resources(requests: &[Value], current_allocations: &Value) -> BTreeMap<String, f64> {
    ...
    if total.is_empty() {
        total.insert("capacity".into(), 100.0);
    }
    total
}
...
fn sort_requests(requests: &[Value]) -> Vec<Value> {
    ...
    out.sort_by(|a, b| {
        b.get("priority")
            .and_then(Value::as_f64)
            .unwrap_or(0.0)
            .partial_cmp(&a.get("priority").and_then(Value::as_f64).unwrap_or(0.0))
...
fn subtract_from(base: &mut BTreeMap<String, f64>, subtract: &BTreeMap<String, f64>) {
    for (k, v) in subtract {
        *base.entry(k.clone()).or_insert(0.0) -= *v;
        if let Some(x) = base.get_mut(k) {
            *x = x.max(0.0);
        }
    }
}
fn object_map(value: &Value) -> BTreeMap<String, BTreeMap<String, f64>> {
    value
        .as_object()
        .map(|o| {
            o.iter()
                .map(|(k, v)| (k.clone(), f64_map_from_value(v)))
                .collect()
        })
        .unwrap_or_default()
}
fn sum_allocations_map(
    allocations: &BTreeMap<String, BTreeMap<String, f64>>,
) -> BTreeMap<String, f64> {
...
fn resource_means(allocations: &BTreeMap<String, BTreeMap<String, f64>>) -> BTreeMap<String, f64> {
    let mut t = sum_allocations_map(allocations);
...
fn balance_score(allocations: &BTreeMap<String, BTreeMap<String, f64>>) -> f64 {
    let totals: Vec<f64> = allocations.values().map(|m| m.values().sum()).collect();
    if totals.is_empty() {
        1.0
    } else {
        let m = mean(&totals);
        let var = totals.iter().map(|v| (v - m).powi(2)).sum::<f64>() / totals.len() as f64;
        1.0 / (1.0 + var.sqrt())
    }
}
fn validate_allocations(allocations: &Value, total: &Value) -> bool {
    let a = sum_allocations_map(&object_map(allocations));
    let t = f64_map_from_value(total);
    a.iter()
        .all(|(k, v)| *v <= t.get(k).cloned().unwrap_or(f64::INFINITY) + 0.001)
}
fn scale_to_total(allocations: &Value, total: &BTreeMap<String, f64>) -> Value {
    let mut maps = object_map(allocations);
    let sum = sum_allocations_map(&maps);
    for (res, actual) in sum {
        let limit = total.get(&res).cloned().unwrap_or(actual);
        if actual > limit && actual > 0.0 {
            let factor = limit / actual;
            for alloc in maps.values_mut() {
    ...
    let obj = maps
        .into_iter()
        .map(|(k, v)| (k, value_from_f64_map(&v)))
        .collect();
    Value::Object(obj)
}
